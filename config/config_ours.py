...
class Config:
    ...
    config_name = 'car-net.new.exp' # car-net.crl, car-net, baseline, baseline.crl
    ...
    loss_param = edict(
        n_classes = 2,
        seg_type = 'wce', # wce, pce, nce, l1, mse
        with_dice = False, 
        ce_weights = [1, 10], 
        reg_weight = 1,
        with_connect = True,
    ...
    test_setting_param = edict(
        model_path = osp.join(model_dir, 'ckpt_best.pth.tar'),
        save_vis = True,
        save_npy = True
        )
